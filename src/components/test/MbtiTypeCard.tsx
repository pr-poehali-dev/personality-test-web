      ...
      <div className={`bg-${color}-600 p-4 text-white`}>
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-${color}-100 mt-1">Тип по Майерс-Бриггс</p>
          </div>
      ...
      <CardContent className="pt-4">
        ...
        <div className="mt-4 grid grid-cols-4 gap-2">
          {type.split('').map((letter, index) => (
            <div key={index} className={`text-center p-2 rounded-md bg-${color}-50 border border-${color}-100`}>
              <span className={`text-${color}-600 font-semibold`}>{letter}</span>
